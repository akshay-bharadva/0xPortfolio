...
import { Bar, BarChart, CartesianGrid, XAxis, Line, LineChart, Pie, PieChart } from "recharts"; // Added more chart types
...
import { SidebarProvider, SidebarTrigger, Sidebar, SidebarHeader, SidebarContent, SidebarFooter, SidebarMenu, SidebarMenuItem, SidebarMenuButton } from "@/components/ui/sidebar"; // Added more sidebar parts for demo
...
const componentSectionsList: ComponentSectionItem[] = [
  ...
  { id: "alertDialog", name: "Alert Dialog", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "alert", name: "Alert", icon: <Terminal className="w-4 h-4 mr-2" />, variations: [{ id: "alert-default", name: "Default" }, { id: "alert-destructive", name: "Destructive" }] },
  ...
  { id: "chart", name: "Chart", icon: <Palette className="w-4 h-4 mr-2" />, variations: [{ id: "chart-bar", name: "Bar Chart" }, { id: "chart-line", name: "Line Chart" }] },
  { id: "checkbox", name: "Checkbox", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "collapsible", name: "Collapsible", icon: <Palette className="w-4 h-4 mr-2" /> },
  ...
  { id: "dialog", name: "Dialog", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "drawer", name: "Drawer", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "dropdownMenu", name: "Dropdown Menu", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "form", name: "Form", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "hoverCard", name: "Hover Card", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "input", name: "Input", icon: <Palette className="w-4 h-4 mr-2" />, variations: [{ id: "input-basic", name: "Basic" }, { id: "input-email", name: "Email" }, { id: "input-disabled", name: "Disabled" }] },
  { id: "inputOtp", name: "Input OTP", icon: <Palette className="w-4 h-4 mr-2" /> },
  ...
  { id: "popover", name: "Popover", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "progress", name: "Progress", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "radioGroup", name: "Radio Group", icon: <Palette className="w-4 h-4 mr-2" /> },
  ...
  { id: "select", name: "Select", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "separator", name: "Separator", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "sheet", name: "Sheet", icon: <Palette className="w-4 h-4 mr-2" />, variations: [{ id: "sheet-sides", name: "Sides" }] },
  { id: "sidebar", name: "Sidebar", icon: <Palette className="w-4 h-4 mr-2" />, variations: [{ id: "sidebar-trigger", name: "Trigger Example" }] },
  { id: "skeleton", name: "Skeleton", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "slider", name: "Slider", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "sonner", name: "Sonner Toasts", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "switch", name: "Switch", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "table", name: "Table", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "tabs", name: "Tabs", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "textarea", name: "Textarea", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "toastShadcn", name: "Toast (Shadcn)", icon: <Palette className="w-4 h-4 mr-2" /> },
  { id: "toggle", name: "Toggle", icon: <Palette className="w-4 h-4 mr-2" />, variations: [{ id: "toggle-variants", name: "Variants" }, { id: "toggle-pressed", name: "Pressed" }] },
  { id: "toggleGroup", name: "Toggle Group", icon: <Palette className="w-4 h-4 mr-2" />, variations: [{ id: "toggle-group-default", name: "Default" }, { id: "toggle-group-outline", name: "Outline" }] },
  { id: "tooltip", name: "Tooltip", icon: <Palette className="w-4 h-4 mr-2" /> },
];
...
const CodeBlock: React.FC<{ code: string; language?: string; title?: string }> = ({ code, language = "tsx", title }) => (
  <div className="my-6 rounded-none border-2 border-black shadow-[4px_4px_0_#000] bg-[#2d2d2d]">
    ...
    <SyntaxHighlighter
      ...
      lineNumberStyle={{ color: '#666', fontSize: '0.8em', marginRight: '1em', userSelect: 'none' }}
      wrapLines={true}
      wrapLongLines={true}
    >
      {code.trim()}
...
const ComponentDocSection: React.FC<{ id: string; title: string; description: string; children: React.ReactNode; className?: string }> = ({ id, title, description, children, className }) => (
  <section id={id} className={cn("mb-16 py-8 px-6 border-2 border-black rounded-none shadow-[8px_8px_0px_#000] bg-white scroll-mt-24", className)}> {/* scroll-mt-24 for fixed header offset */}
...
const VariationDisplay: React.FC<{ id?: string; title: string; description?: string; children: React.ReactNode; className?: string }> = ({ id, title, description, children, className }) => (
  <div id={id} className={cn("mt-6 p-4 border-2 border-gray-400 rounded-none shadow-[3px_3px_0px_#aaa] bg-gray-50 scroll-mt-24", className)}> {/* scroll-mt-24 for fixed header offset */}
      ...
      <TableBody>
        {data.map(row => (
          <TableRow key={row.prop}>
            <TableCell className="font-mono font-semibold text-sm">{row.prop}</TableCell>
            <TableCell className="font-mono text-xs text-indigo-700">{row.type}</TableCell>
...
export default function UiDocumentationPage() {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            const mainComponentId = componentSectionsList.find(comp => comp.id === entry.target.id || comp.variations?.some(v => v.id === entry.target.id))?.id;
  ...
  return (
    <Layout>
      ...
      <style jsx global>{`
            :root { --header-height: 80px; } /* Example, adjust to your actual fixed header height */
      ...
      <div className="flex flex-col md:flex-row min-h-[calc(100vh-var(--header-height))] font-space bg-gray-100">
              ...
              {componentSectionsList.map(section => (
                <li key={section.id}>
                  ...
                  {section.variations && activeSidebarSection === section.id && (
                    <ul className="pl-3 mt-0.5 mb-1 space-y-0.5 border-l-2 border-gray-400 ml-3">
                      {section.variations.map(variation => (
                        <li key={variation.id}>
                          <button
                            onClick={() => scrollToSectionHandler(variation.id)}
                            className="flex items-center w-full text-left px-2 py-1 rounded-none text-xs font-semibold transition-colors duration-100 text-gray-700 hover:text-black hover:bg-yellow-100"
                          >
                            <span className="mr-2">-</span>{variation.name}
                          </button>
                        </li>
                      ))}
                    </ul>
        ...
        <main ref={mainContentAreaRef} className="w-full md:w-[70%] p-4 sm:p-6 md:p-8 lg:p-10 overflow-y-auto">
          ...
          >
            ...
            <VariationDisplay id="accordion-basic" title="Basic Usage (Single Item Collapsible)">
              <Accordion type="single" collapsible className="w-full max-w-md">
                <AccordionItem value="item-1"><AccordionTrigger>What is Neo-Brutalism?</AccordionTrigger><AccordionContent>Neo-Brutalism in UI design emphasizes raw, blocky elements, strong typography, limited color palettes (often high contrast), and an unrefined, functional aesthetic. It often features hard shadows and visible borders.</AccordionContent></AccordionItem>
                <AccordionItem value="item-2"><AccordionTrigger>Is it accessible?</AccordionTrigger><AccordionContent>Yes. It adheres to the WAI-ARIA design pattern.</AccordionContent></AccordionItem>
              </Accordion>
            </VariationDisplay>
            ...
            <VariationDisplay id="accordion-multiple" title="Multiple Items Openable">
              <Accordion type="multiple" className="w-full max-w-md">
                <AccordionItem value="item-a"><AccordionTrigger>Item One (Multiple)</AccordionTrigger><AccordionContent>Content one.</AccordionContent></AccordionItem>
                <AccordionItem value="item-b"><AccordionTrigger>Item Two (Multiple)</AccordionTrigger><AccordionContent>Content two.</AccordionContent></AccordionItem>
              </Accordion>
            </VariationDisplay>
            <CodeBlock code={`<Accordion type="multiple">{/* ...items... */}</Accordion>`} />
            <PropsTable data={[
              { prop: "type", type: "'single' | 'multiple'", default: "'single'", description: "Determines if multiple items can be open simultaneously." },
              { prop: "collapsible", type: "boolean", default: "false", description: "If type='single', allows the open item to be closed by clicking its trigger again." },
              { prop: "defaultValue", type: "string | string[]", description: "The value(s) of the item(s) to be open initially." },
              { prop: "value", type: "string | string[]", description: "Controlled open item(s)." },
              { prop: "onValueChange", type: "(value: string | string[]) => void", description: "Callback when open items change." },
            ]} />
          </ComponentDocSection>
          ...
          >
            ...
            <VariationDisplay id="button-variants" title="Variants" description="Different visual styles for various purposes.">
            ...
            <VariationDisplay id="button-sizes" title="Sizes" description="Available button sizes.">
              <Button size="sm">Small</Button>
              <Button size="default">Default Size</Button>
              <Button size="lg">Large</Button>
              <Button size="icon" aria-label="Settings"><Settings className="h-5 w-5" /></Button>
            </VariationDisplay>
            <CodeBlock code={`<Button size="sm">Small</Button>\n<Button size="default">Default</Button>\n<Button size="lg">Large</Button>\n<Button size="icon"><Icon /></Button>`} />

            <VariationDisplay id="button-icon" title="With Icon" description="Buttons can include icons for better context.">
              <Button><UserIcon className="mr-2 h-4 w-4" /> Login with Email</Button>
              <Button variant="outline"><Settings className="mr-2 h-4 w-4" /> Settings</Button>
            </VariationDisplay>
            <CodeBlock code={`<Button><UserIcon className="mr-2 h-4 w-4" /> Login</Button>`} />

            <VariationDisplay id="button-disabled" title="Disabled State" description="Visually indicates that the button is not interactive.">
            ...
            <VariationDisplay id="button-aschild" title="As Child (Polymorphic)" description="Render the button as a different HTML element, useful for links.">
              <Button asChild variant="link" className="text-lg">
                <a href="#button">Link Styled as Button</a>
              </Button>
            </VariationDisplay>
            <CodeBlock code={`<Button asChild variant="link">\n  <a href="/some-page">Navigation Link</a>\n</Button>`} />
            <PropsTable data={[
              { prop: "variant", type: "'default'|'destructive'|'outline'|'secondary'|'ghost'|'link'", default: "'default'", description: "Button style variant." },
              { prop: "size", type: "'default'|'sm'|'lg'|'icon'", default: "'default'", description: "Button size." },
              { prop: "asChild", type: "boolean", default: "false", description: "Render as a child element, inheriting props." },
              { prop: "disabled", type: "boolean", default: "false", description: "Disables button interaction." },
            ]} />
          </ComponentDocSection>
          ...
          >
            <p className="text-sm mb-4">Import: <code className="text-xs bg-gray-200 p-1 border border-black rounded-none">import {'{ Card, CardHeader, CardTitle, CardDescription, CardContent, CardFooter }'} from "@/components/ui/card";</code></p>
            <VariationDisplay id="card-basic" title="Basic Card Structure">
              <Card className="w-full max-w-sm">
                ...
                <CardContent>
                  <p>This card contains essential information about Project Alpha, including its goals and expected outcomes. The content area can hold various elements like text, lists, or even images.</p>
                </CardContent>
...
<Card>
  ...
  <CardFooter>...</CardFooter>
            ...
            `} />
            <div className="mt-4 p-3 bg-yellow-100 border-2 border-yellow-400 rounded-none shadow-[2px_2px_0_#000]">
              <p className="text-sm font-semibold text-yellow-800"><strong className="font-black">Anatomy:</strong></p>
              <ul className="list-disc list-inside text-xs text-yellow-700 mt-1">
                <li>`Card`: The main container.</li>
                <li>`CardHeader`: Optional header section, often contains `CardTitle` and `CardDescription`.</li>
                ...
                <li>`CardFooter`: Optional footer section, typically for actions.</li>
              </ul>
            </div>
          </ComponentDocSection>
          ...
          >
            <p className="text-sm mb-4">Import: <code className="text-xs bg-gray-200 p-1 border border-black rounded-none">import {'{ InputOTP, InputOTPGroup, InputOTPSlot, InputOTPSeparator }'} from "@/components/ui/input-otp";</code></p>
            <VariationDisplay id="inputOtp-basic" title="Basic 6-Digit OTP Input">
              <div className="flex flex-col items-center space-y-2">
                ...
                <p className="text-sm text-gray-600 font-semibold">Current Value: <span className="font-mono text-black">{otp || "----"}</span></p>
                <Button size="sm" variant="outline" onClick={() => setOtp("")} disabled={!otp}>Clear</Button>
              </div>
            </VariationDisplay>
            <CodeBlock code={`
...
<InputOTP maxLength={6} value={otpValue} onChange={(value) => setOtpValue(value)}>
  ...
  <InputOTPGroup>
    ...
    <InputOTPSlot index={5} />
            ...
            `} />
            <VariationDisplay id="inputOtp-no-separator" title="OTP Input without Separator">
...
<InputOTP maxLength={4}>
  <InputOTPGroup>
    ...
    <InputOTPSlot index={3} />
            ...
            `} />
            <PropsTable data={[
              { prop: "maxLength", type: "number", default: "6 (usually)", description: "Maximum number of characters for the OTP." },
              { prop: "value", type: "string", description: "Controlled value of the OTP input." },
              { prop: "onChange", type: "(value: string) => void", description: "Callback when the OTP value changes." },
              { prop: "pattern", type: "string", description: "RegExp pattern for input validation (e.g., REGEXP_ONLY_DIGITS)." },
              { prop: "containerClassName", type: "string", description: "Class name for the root OTPInput container." },
              { prop: "disabled", type: "boolean", default: "false", description: "Disables the input field." },
              { prop: "children", type: "ReactNode", description: "Typically InputOTPGroup and InputOTPSeparator." },
            ]} />
            <div className="mt-4 p-3 bg-yellow-100 border-2 border-yellow-400 rounded-none shadow-[2px_2px_0_#000]">
              <p className="text-sm font-semibold text-yellow-800"><strong className="font-black">Child Components:</strong></p>
              <ul className="list-disc list-inside text-xs text-yellow-700 mt-1">
                <li>`InputOTPGroup`: Wraps a group of `InputOTPSlot`s.</li>
                <li>`InputOTPSlot`: Represents an individual character slot. Requires an `index` prop.</li>
                <li>`InputOTPSeparator`: Optional visual separator between groups of slots.</li>
              </ul>
            </div>
          </ComponentDocSection>
          ...
          >
            <p className="text-sm mb-4">Import: <code className="text-xs bg-gray-200 p-1 border border-black rounded-none">import {'{ Label }'} from "@/components/ui/label";</code></p>
            <VariationDisplay id="label-basic" title="Basic Label with Input">
              <div className="grid w-full max-w-sm items-center gap-1.5">
                <Label htmlFor="email-label">Email Address</Label>
                <Input type="email" id="email-label" placeholder="you@example.com" />
              </div>
            </VariationDisplay>
            <CodeBlock code={`
...
<Input type="email" id="email" placeholder="Email" />
            ...
            <VariationDisplay id="label-checkbox" title="Label with Checkbox">
              <div className="flex items-center space-x-2">
                ...
                <Label htmlFor="terms-label" className="font-semibold">Accept our awesome terms</Label>
              </div>
            </VariationDisplay>
            <CodeBlock code={`
<Checkbox id="terms" />
<Label htmlFor="terms">Accept terms and conditions</Label>
            `} />
            <PropsTable data={[
              { prop: "htmlFor", type: "string", description: "The ID of the form control the label is associated with." },
              { prop: "className", type: "string", description: "Additional CSS classes for styling." },
            ]} />
          </ComponentDocSection>
          ...
          >
            <p className="text-sm mb-4">Import: <code className="text-xs bg-gray-200 p-1 border border-black rounded-none">import {'{ Menubar, MenubarMenu, MenubarTrigger, ... }'} from "@/components/ui/menubar";</code></p>
              ...
              <Menubar>
                <MenubarMenu>
                  ...
                  <MenubarContent>
                    ...
                    <MenubarSeparator />
                    <MenubarSub><MenubarSubTrigger>Share</MenubarSubTrigger>
                      <MenubarSubContent>
                ...
                <MenubarMenu>
                  ...
                  <MenubarContent>
                    ...
                    <MenubarSub><MenubarSubTrigger>Find</MenubarSubTrigger>
                      <MenubarSubContent><MenubarItem>Search the web...</MenubarItem><MenubarSeparator /><MenubarItem>Find...</MenubarItem></MenubarSubContent>
                    </MenubarSub>
                    <MenubarSeparator />
...
<Menubar>
  ...
  {/* ... more MenubarMenu ... */}
            ...
            `} />
            <div className="mt-4 p-3 bg-yellow-100 border-2 border-yellow-400 rounded-none shadow-[2px_2px_0_#000]">
              <p className="text-sm font-semibold text-yellow-800"><strong className="font-black">Key Parts:</strong></p>
              <ul className="list-disc list-inside text-xs text-yellow-700 mt-1">
                <li>`Menubar`: The root container for the menubar.</li>
                <li>`MenubarMenu`: Container for a single top-level menu (e.g., "File", "Edit").</li>
                <li>`MenubarTrigger`: The button that opens a `MenubarContent`.</li>
                <li>`MenubarContent`: The dropdown panel containing menu items.</li>
                <li>`MenubarItem`: A command item within the menu. Can include `MenubarShortcut`.</li>
                <li>`MenubarSeparator`: A visual divider between groups of items.</li>
                <li>`MenubarSub`, `MenubarSubTrigger`, `MenubarSubContent`: For creating nested sub-menus.</li>
                <li>`MenubarCheckboxItem`, `MenubarRadioGroup`, `MenubarRadioItem`: Specialized menu items.</li>
              </ul>
            </div>
          </ComponentDocSection>
          ...
          >
            <p className="text-sm mb-4">Import: <code className="text-xs bg-gray-200 p-1 border border-black rounded-none">import {'{ NavigationMenu, NavigationMenuList, ... }'} from "@/components/ui/navigation-menu";</code></p>
            <VariationDisplay id="navigationMenu-example" title="Example Navigation Menu">
              <NavigationMenu>
                <NavigationMenuList>
                  <NavigationMenuItem>
                    <NavigationMenuTrigger>Learn</NavigationMenuTrigger>
                    <NavigationMenuContent>
                      <ul className="grid gap-3 p-4 md:w-[400px] lg:w-[500px] lg:grid-cols-[.75fr_1fr] bg-white border-2 border-black rounded-none shadow-[3px_3px_0_#000]">
                        <li className="row-span-3">
                          <NavigationMenuLink asChild>
                            <a className="flex h-full w-full select-none flex-col justify-end rounded-none border-2 border-black bg-gradient-to-b from-gray-100 to-gray-200 p-6 no-underline outline-none focus:shadow-md shadow-[2px_2px_0_#000] hover:bg-yellow-100" href="#navigationMenu">
                              <HomeIcon className="h-6 w-6 text-black" />
                              <div className="mb-2 mt-4 text-lg font-bold text-black">UI Kit Home</div>
                              <p className="text-sm leading-tight text-gray-700">A beautiful set of components for your next project.</p>
                            </a>
                          </NavigationMenuLink>
                        </li>
                        <ListItemUi href="#button" title="Buttons">Robust and versatile button components.</ListItemUi>
                        <ListItemUi href="#card" title="Cards">Containers for content and actions.</ListItemUi>
                        <ListItemUi href="#input" title="Inputs">Standard form input fields.</ListItemUi>
                      </ul>
                    </NavigationMenuContent>
                  </NavigationMenuItem>
                  <NavigationMenuItem>
                    ...
                    <NavigationMenuContent>
                      <ul className="grid w-[300px] gap-3 p-4 md:w-[400px] md:grid-cols-2 bg-white border-2 border-black rounded-none shadow-[3px_3px_0_#000]">
                        <ListItemUi href="#alert" title="Alert">Attention-grabbing messages.</ListItemUi>
                        <ListItemUi href="#dialog" title="Dialog">Modal confirmation windows.</ListItemUi>
                        <ListItemUi href="#form" title="Forms">Build accessible forms easily.</ListItemUi>
                      </ul>
                    </NavigationMenuContent>
                  </NavigationMenuItem>
                  <NavigationMenuItem>
                    <a href="#navigationMenu" className={cn(navigationMenuTriggerStyle(), "font-bold")}>Documentation</a>
                  </NavigationMenuItem>
                </NavigationMenuList>
              </NavigationMenu>
            </VariationDisplay>
  ...
  ({ className, title, children, ...props }, ref) => { /* ...implementation... */ }
            ...
            <div className="mt-4 p-3 bg-yellow-100 border-2 border-yellow-400 rounded-none shadow-[2px_2px_0_#000]">
              ...
              <ul className="list-disc list-inside text-xs text-yellow-700 mt-1">
                ...
                <li>`NavigationMenuList`: Contains the list of top-level menu items.</li>
                <li>`NavigationMenuItem`: Wraps each top-level item (trigger or link).</li>
                <li>`NavigationMenuTrigger`: A button that toggles a `NavigationMenuContent` dropdown.</li>
                <li>`NavigationMenuContent`: The dropdown panel. Often contains a `ul` with `ListItemUi` (or similar custom components).</li>
                <li>`NavigationMenuLink`: A styled link. Can be used directly or with `navigationMenuTriggerStyle()`.</li>
              </ul>
            </div>
          </ComponentDocSection>
          ...
          >
            <p className="text-sm mb-4">Import: <code className="text-xs bg-gray-200 p-1 border border-black rounded-none">import {'{ Pagination, PaginationContent, ... }'} from "@/components/ui/pagination";</code></p>
    ...
    <PaginationItem><PaginationNext href="#" /></PaginationItem>
            ...
            <div className="mt-4 p-3 bg-yellow-100 border-2 border-yellow-400 rounded-none shadow-[2px_2px_0_#000]">
              ...
              <ul className="list-disc list-inside text-xs text-yellow-700 mt-1">
                ...
                <li>`PaginationPrevious`, `PaginationNext`: Buttons for previous/next page.</li>
                <li>`PaginationLink`: A page number link. Use `isActive` prop for the current page.</li>
          ...
          <section className="mt-16 py-10 text-center border-2 border-dashed border-gray-400 rounded-none bg-gray-100">
            <h2 className="text-2xl font-bold text-gray-600">Further Component Documentation Pending...</h2>
            <p className="text-gray-500">This showcase is a work in progress. Follow the established pattern to document remaining components.</p>
          </section>
