...
export default function SupabaseMFAChallenge() {
  ...
  useEffect(() => {
    const protectPageAndGetFactor = async () => {
      ...
      if (aalData?.currentLevel !== "aal1" || aalData?.nextLevel !== "aal2") {
        ...
        // Intentionally not setting isLoadingState to false here if redirecting immediately.
        router.replace("/admin/login");
        return;
      }
      ...
      if (factorsError || !factorsData?.totp?.length) {
        ...
        setIsLoadingState(false); // Set loading to false before redirect on error
        router.replace("/admin/login");
        return;
      }
  ...
  // Show main loader only if no early error has occurred and page is generally loading factors.
  ...
  return (
    ...
    >
      <div className="w-full max-w-md space-y-8 border-2 border-black bg-white p-8 shadow-[8px_8px_0px_#000000]">
        ...
        <div className="rounded-none bg-white p-0 md:p-4">
